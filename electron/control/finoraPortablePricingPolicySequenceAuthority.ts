 ...
 * PORTABLE PRICING_POLICY RECIPIENT SEQUENCE AUTHORITY
 *
 ...
 *
 * Portable PRICING_POLICY sequence authority is branch scoped.
 * Historical native PRICING_POLICY rows remain installation
...
export interface FinoraPortablePricingPolicyAppliedPackageRecord {
  packageId:
    string;
}

export interface FinoraPortablePricingPolicyLegacySequenceRecord {
  issuerId:
    string;
...
export interface EvaluateFinoraPortablePricingPolicySequenceInput {
  packageId:
    string;
...
export type FinoraPortablePricingPolicySequenceEvaluation =
  | {
      success:
        true;
...
export function evaluateFinoraPortablePricingPolicySequence(
  input:
    EvaluateFinoraPortablePricingPolicySequenceInput,
): FinoraPortablePricingPolicySequenceEvaluation {
  ...
  /*
   * Legacy PRICING_POLICY sequence rows are installation scoped.
   *
  ...
  for (
    const record of
      input.portablePricingPolicySequences
  ) {
