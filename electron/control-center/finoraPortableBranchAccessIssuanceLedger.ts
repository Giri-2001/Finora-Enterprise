...
import {
  getFinoraHistoricalBranchAccessIssuanceHighWater,
} from "./finoraControlCenterIssuanceLedger.js";
export const FINORA_PORTABLE_BRANCH_ACCESS_PURPOSE =
  "BRANCH_ACCESS" as const;

export interface FinoraPortableBranchAccessIssuanceScope {
  ownerId:
    string;
...
export interface FinoraPortableBranchAccessIssuanceReservation {
  packageId:
    string;
...
interface FinoraPortableBranchAccessSequenceRecord {
  ...
  purpose:
    typeof FINORA_PORTABLE_BRANCH_ACCESS_PURPOSE;
...
function getLedgerPath():
  ...
  if (!app.isReady()) {
    throw new Error(
      "FINORA Portable Branch Access issuance ledger cannot be used before Electron is ready.",
    );
  }
...
function isIssuanceScope(
  value:
    unknown,
): value is FinoraPortableBranchAccessIssuanceScope {
...
function isSequenceRecord(
  value:
    unknown,
): value is FinoraPortableBranchAccessSequenceRecord {
  ...
  return (
    ...
    value.purpose ===
      FINORA_PORTABLE_BRANCH_ACCESS_PURPOSE &&
    isNonEmptyString(
      value.ownerId,
    ) &&
    isNonEmptyString(
...
function isLedger(
  value:
    unknown,
): value is FinoraPortableBranchAccessIssuanceLedger {
...
function validateReservationScope(
  ...
  if (!isIssuanceScope(scope)) {
    throw new Error(
      "FINORA Portable Branch Access issuance scope must contain exactly ownerId, businessId, and branchId.",
    );
  }
}
...
function createEmptyLedger(
  ...
  FinoraPortableBranchAccessIssuanceLedger {
...
async function readLedger():
  ...
  if (
    ...
    throw new Error(
      "FINORA secure Portable Branch Access issuance-ledger encryption is unavailable.",
    );
  }
  ...
  try {
    ...
    throw new Error(
      "FINORA Portable Branch Access issuance ledger cannot be decrypted.",
    );
  }
  ...
  try {
    ...
    throw new Error(
      "FINORA Portable Branch Access issuance ledger is corrupt.",
    );
  }

  if (!isLedger(parsed)) {
    throw new Error(
      "FINORA Portable Branch Access issuance ledger schema is invalid.",
    );
  }
...
async function writeLedger(
  ...
  if (
    ...
    throw new Error(
      "FINORA secure Portable Branch Access issuance-ledger encryption is unavailable. Reservation was not persisted.",
    );
  }

  if (!isLedger(ledger)) {
    throw new Error(
      "FINORA Portable Branch Access issuance ledger failed schema validation.",
    );
  }
...
async function reserveInternal(
  ...
  FinoraPortableBranchAccessIssuanceReservation
> {
  ...
  /*
   * BRANCH_ACCESS existed before device portability and its
   ...
   * advance the legacy BRANCH_ACCESS namespace.
   */
  const historicalHighWater =
    await getFinoraHistoricalBranchAccessIssuanceHighWater({
      issuerId,
  ...
  if (
    ...
    throw new Error(
      "FINORA Portable Branch Access issuance sequence is exhausted.",
    );
  }
  ...
  return {
    packageId:
      `FINORA-PORTABLE-BRANCH-ACCESS-${randomUUID()}`,
...
> {
  ...
  const operation =
    portableBranchAccessIssuanceQueue.then(
      () =>
        reserveInternal(
          scope,
        ),
  ...
  portableBranchAccessIssuanceQueue =
    operation.then(
      () =>
        undefined,
      () =>
